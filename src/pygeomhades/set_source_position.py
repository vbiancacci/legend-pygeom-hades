...
def ask_and_print_runs(node: Any) -> None:
    ans = (
        input("Do you want to see the full list of available runs, runXXXX: [phi, x, z] ? [y/N]: ")
        .strip()
        .lower()
    )
    if ans in ("y", "yes"):
...
def check_source_position(
    ...
    if phi_position not in phi_pos:
        print(
            f"Position ERROR: Provided phi position {phi_position} not found in the database for the given measurement {hpge_name}/{campaign}/{measurement}.\n"
        ...
        if ans in ("y", "yes"):
            for run in node.keys():
                print(f"{run}:  {list(getattr(node, run).source_position.values())}")
        raise ValueError(
            f"Provided phi position {phi_position} not found in the database for the given measurement {hpge_name}/{campaign}/{measurement}."
        )
    ...
    if not any(matched_r):
        print(
            f"Position ERROR: Provided r position {r_position} not found in the database for the given measurement {hpge_name}/{campaign}/{measurement}.\n"
            "For the provided phi position " + str(phi_position) + "\n"
            "the available r positions are: " + str(r_available)
        )
        ask_and_print_runs(node)
        raise ValueError(
            f"Provided r position {r_position} not found in the database for the given measurement  {hpge_name}/{campaign}/{measurement}."
        )
    if matched_z == False:
        print(
            f"Position ERROR: Provided z position {z_position} not found in the database for the given measurement  {hpge_name}/{campaign}/{measurement}.\n"
        ...
        ask_and_print_runs(node)
        raise ValueError(
            f"Provided z position {z_position} not found in the database for the given measurement  {hpge_name}/{campaign}/{measurement}."
        )
...
def set_source_position(config: dict) -> tuple[str, list, list]:
    ...
    if not os.path.isfile(MeasurementPath):
        raise FileNotFoundError(
            f"The measurement {MeasurementPath} does not exist. Please check the configuration file and metadata."
        )
    ...
    if run.isdigit():  # the user knows the run number
        ...
        try:
            ...
            raise ValueError(
                f"Run '{run}' not found in the metadata. Run names must be 4-digit strings with leading zeros, e.g., '0001'."
            )
